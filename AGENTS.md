...
4. Softbuilding:
    ...
    c. Run a targeted check only when it is needed to diagnose or isolate a specific implementation failure, then still finish with the single soft build.
    d. This rule applies in build mode, not plan mode; you cannot activate or rebuild the system yourself because that requires sudo.
    e. Do not run a softbuild if it is expected to need to compile a package from source locally. In that event, alert me that the changes will require a longer nix rebuild time.
